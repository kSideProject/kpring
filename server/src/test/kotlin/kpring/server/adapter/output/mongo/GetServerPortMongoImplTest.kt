...
class GetServerPortMongoImplTest(
  ...
  it("저장된 서버의 정보를 조회할 수 있다.") {
    // given
    val serverUserEntities = mutableListOf(
      ServerUserEntity("id", "name", "/path")
    )
    val serverEntity = serverRepository.save(
    ...
    server.users[0].id shouldBe "id"
    server.users[0].name shouldBe "name"
    server.users[0].profileImage shouldBe "/path"
  }
